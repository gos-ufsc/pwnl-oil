...
function solve_and_store(platform, scenario_id, instance_id; 
                        results_dir="results", time_budget=30.0)
    ...
    try
        ...
        for var in all_variables(model)
            if startswith(name(var), "ξ")
                n_ξ += 1
            end
            if startswith(name(var), "λ")
                n_λ += 1
            end
            if is_binary(var)
                n_bin += 1
        ...
        println("Model has $n_ξ ξ variables, $n_λ λ variables, and $n_bin binary variables.")

        set_time_limit_sec(model, 10.0)  # Set time limit for Gurobi
        optimize!(model)
